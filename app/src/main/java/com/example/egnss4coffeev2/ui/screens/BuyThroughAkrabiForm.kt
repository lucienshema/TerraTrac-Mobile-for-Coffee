...
//        )
        ...
        Button(
            onClick = {
                ...
                navController.navigate("bought_items")
            },
            modifier = Modifier.fillMaxWidth()
        ) {
            Text("Submit")
...
fun EditAkrabiScreen(
    akrabiId: Long,
    viewModel: AkrabiViewModel = viewModel(),
    navController: NavController
) {
    // Observe LiveData from ViewModel
    val akrabi by viewModel.getAkrabiById(akrabiId).observeAsState(null)

    // Display a loading indicator while waiting for data
    if (akrabi == null) {
        // You can use any loading indicator or placeholder here
